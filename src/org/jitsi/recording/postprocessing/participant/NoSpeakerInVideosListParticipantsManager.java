...
/**
 * An implementation of {@link ActiveParticipantsManager} that doesn't show
 * the active speaker in the active participants list
 * @author vmarinov
 */
public class NoSpeakerInVideosListParticipantsManager 
    implements ActiveParticipantsManager 
{
    ...
    public void removeParticipant(int participantSSRC)
    {
        ...
        if (isActiveSpeaker) 
        {
            ...
            if (participants.size() > MAX_ACTIVE_PARTICIPANTS_NUMBER) 
            {
                ...
                swapActiveParticipants(activeParticipantsIndex, 
                    latestActiveNonSpeakerIndex);
                activeParticipants.set(latestActiveNonSpeakerIndex, 
                    participants.get(toBecomeActiveIndex));
                activeParticipants.get(activeParticipantsIndex).
    ...
    public void speakerChange(int newSpeakerSSRC, int eventInstant)
    {
        ...
        for (int i = 0; i < activeParticipants.size(); i++)
        {
            ...
            if (activeParticipants.get(i).isCurrentlySpeaking) 
            {
        ...
        if (activeParticipantsIndex > -1)
        {
            ...
            {
                swapActiveParticipants(
                    activeSpeakerIndex, activeParticipantsIndex);
            }
        } else 
        {
            ...
            swapActiveParticipants(activeSpeakerIndex, oldestActiveIndex);
        }
    ...
    {
        int resultIndex = -1, latestInstant = 0;
    ...
    /**
     * Swaps two active participants
     * @param index1 Index of the first participant
     * @param index2 Index of the second participant
     */
    private void swapActiveParticipants(int index1, int index2)
    {
        ParticipantInfo temp = activeParticipants.get(index1);
            activeParticipants.set(index1, activeParticipants.get(index2));
            activeParticipants.set(index2, temp);
    }
